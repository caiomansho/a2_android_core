...
public class CurrencyValidatorInputType implements ValidatorInputType {

    private EditText editText;
    private Context context;

    public CurrencyValidatorInputType(Context context, EditText editText){
    ...
    public Boolean isValid() {
        if (!editText.getText().toString().isEmpty() && (editText.getMustBeGreaterThenZero() && !(CurrencyMaskTextWatcher.toFloat(editText) > 0))) {
