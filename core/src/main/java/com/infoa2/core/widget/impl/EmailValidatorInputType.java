...
public class EmailValidatorInputType implements ValidatorInputType {

    private EditText editText;
    private Context context;

    public EmailValidatorInputType(Context context, EditText editText) {
    ...
    public Boolean isValid() {
        if (!editText.getText().toString().isEmpty() && !editText.isEmail()) {
            ...
            if (editText.getCustomErrorMessageId() != 0) {
                ...
                editText.setError(context.getString(R.string.sou_widgets_invalid_email));
