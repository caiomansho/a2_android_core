 ...
 */
class PercentageValidatorInputType(private val context: Context, private val editText: EditText) : ValidatorInputType {

    override val isValid: Boolean
        get() {
            if (!editText.text!!.toString().isEmpty() && editText.mustBeGreaterThenZero!! && PercentageMaskTextWatcher.toFloat(
                    editText
                ) <= 0
            ) {
